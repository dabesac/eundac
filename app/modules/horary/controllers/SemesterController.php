...
class Horary_SemesterController extends Zend_Controller_Action{
	...
	public function semesterlistAction(){
		try {
			...
			$eid=$this->sesion->eid;
			$oid=$this->sesion->oid;
			$sesion_escid=base64_encode($this->sesion->escid);
			$sesion_subid=base64_encode($this->sesion->subid);
			$escid=base64_decode($this->_getParam('escid',$sesion_escid));
			$subid=base64_decode($this->_getParam('subid',$sesion_subid));
	...
	public function printhorarysemesterAction(){
        ...
        if ($datahours) {
            ...
            $wheri = array('eid'=>$eid,'oid'=>$oid,'escid'=>$escid,'subid'=>$subid,'type_impression'=>$nameimpre,'perid'=>$perid);
