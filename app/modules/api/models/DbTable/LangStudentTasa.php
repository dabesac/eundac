<?php
class Api_Model_DbTable_LangStudentTasa extends Zend_Db_Table_Abstract{

	protected $_name = 'lang_student_tasa';
	protected $_primary = array('eid', 'perid', 'pid', 'subid');
  	...
  	public function _getFilter($where=null,$attrib=null,$orders=null){
		try{
	        if($where['eid']=='') return false;
	          ...
	          if ($attrib=='') $select->from("lang_student_tasa");
	          else $select->from("lang_student_tasa",$attrib);
    ...
    public function _save($data){
		try {	
			if ($data['eid'] == '' || $data['perid'] == '' || $data['pid'] == '' || $data['subid'] == '') return false;
